...
August ended, and we shipped. Not because we'd built something perfect. But because we'd built something that worked, that scaled, and that we understood.

Looking back on three months, the most important thing I learned had nothing to do with RAG or vector databases or eval pipelines. It was simpler: building a product with AI means obsessing over what actually matters to users, even when it's invisible in the code.
...
A tutoring system that's fast and teaches isn't a technical problem waiting for the right architecture. It's a design problem. It requires asking: what does good tutoring actually look like? What are we optimizing for? What are we willing to trade?

Most LLM products start with a technical decision (use GPT-4, use RAG, use fine-tuning). The real work is figuring out what "good" means for your users, then building infrastructure to measure and protect it.
...
Here's what I've learned: the architecture matters. But what matters more is measurement. Once you can measure what you care about, you can iterate. You can catch problems before they ship. You can make real tradeoffs.

Every LLM product has this challenge. A customer support chatbot needs to be helpful, not just informative. A content generator needs to match brand voice, not just be grammatically correct. A coding assistant needs to teach, not just generate.
...
That's the eval question. And it's bigger than evals.
...
But we had something valuable: we all cared about the product. We weren't chasing the coolest architecture. We were chasing something that actually worked for students.

That mattered more than any individual decision. We'd debate about Chroma vs. other databases, about chunk sizes, about prompt strategies. But underneath was one shared thing: what do students actually need?

That alignment—around purpose, not implementation—made the difference. It's also what's hardest to hire for or build into a team. But it matters.

## What I'd Do Differently

If I built this again, knowing what I know now:

**I'd measure earlier.** We didn't build the eval pipeline until halfway through. If we'd started with "here's what good looks like, here's how we measure it," we would have caught the teaching problem sooner and wasted less time on slowness optimization.

**I'd focus less on metrics, more on users.** We spent a lot of time looking at precision, recall, latency numbers. We spent less time watching actual students use the system. Watching a student give up waiting for an answer taught us more than any metric.

**I'd accept uncertainty longer.** We wanted to be right quickly. But the first month, we should have just explored. What works? What doesn't? What's surprising? We could have shipped scrappier, learned faster, iterated more.
